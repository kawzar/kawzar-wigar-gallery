...
app.get('/coc', (request, response) => {
    response.sendFile('./public/coc.html', { root: __dirname })
})
...
io.on('connection', function(socket) {
    ...
    socket.on('join', function(playerInfo) {
        ...
        try {
            ...
            if (playerInfo.nickName == '')
                console.log(
                    'New user joined the server in lurking mode ' + socket.id + ' ' + IP,
                )
            else
                console.log(
            ...
            if (myRoom != undefined) {
                ...
                console.log(
                    'There are now ' + roomPlayers + ' users in ' + playerInfo.room,
                )
            }
            ...
            //prevent a hacked client from duplicating players
                ...
                console.log(
                    'ATTENTION: there is already a player associated to the socket ' +
                    socket.id,
                )
            } else if (
                (serverPlayers > MAX_PLAYERS && MAX_PLAYERS != -1) ||
                (roomPlayers > MAX_PLAYERS_PER_ROOM && MAX_PLAYERS_PER_ROOM != -1)
            ) {
                //limit the number of players
                console.log(
                    'ATTENTION: ' + playerInfo.room + ' reached maximum capacity',
                )
                socket.emit(
                    'errorMessage',
                ...
                if (playerInfo.nickName.length > MAX_NAME_LENGTH)
                    playerInfo.nickName = playerInfo.nickName.substring(
                        0,
                        MAX_NAME_LENGTH,
                    )
                ...
                if (playerInfo.nickName != '') val = validateName(playerInfo.nickName)

                if (val == 0 || val == 3) {
                    console.log(
                        'ATTENTION: ' + socket.id + ' tried to bypass username validation',
                    )
                } else {
                    ...
                    console.log(
                        ...
                        ' players on this server. Total visits ' +
                        visits,
                    )

                    io.sockets.emit('playerAmount', { current: Object.keys(gameState.players).length, total: visits });
                }
            }
        } catch (e) {
    ...
    socket.on('disconnect', function() {
        try {
            ...
            console.log(
                'Error on disconnect, object malformed from' + socket.id + '?',
            )
            console.error(e)
        }
    })
    ...
    socket.on('talk', function(obj) {
        try {
            ...
            //block if spamming
            if (
                time - gameState.players[socket.id].lastMessage >
                DATA.SETTINGS.ANTI_SPAM &&
                !gameState.players[socket.id].muted
            ) {
                //Admin commands can be typed as messages
                //is this an admin
                if (
                    gameState.players[socket.id].admin &&
                    obj.message.charAt(0) == '/'
                ) {
                    console.log(
                        'Admin ' +
                        gameState.players[socket.id].nickName +
                        ' attempts command ' +
                        obj.message,
                    )
                    ...
                    if (
                        filter.isProfane(test) ||
                        filter.isProfane(test2) ||
                        filter.isProfane(test3) ||
                        test4 == ''
                    ) {
                        ...
                        if (MOD[obj.room + 'TalkFilter'] != null) {
                            //call it!
                            obj.message = MOD[obj.room + 'TalkFilter'](
                            ...
                            if (obj.message == null) {
                                console.log(
                                    'MOD: Warning - TalkFilter should return a message ',
                                )
                                obj.message = ''
                            }
                        }
...
setInterval(function() {
    ...
    for (var id in gameState.players) {
        if (gameState.players.hasOwnProperty(id)) {
            if (
                gameState.players[id].nickName != '' &&
                time - gameState.players[id].lastActivity > ACTIVITY_TIMEOUT
            ) {
                console.log(
                    id +
                    ' has been idle for more than ' +
                    ACTIVITY_TIMEOUT +
                    ' disconnecting',
                )
...
global.NPC = function(o) {
    console.log(
        'Create NPC ' + o.id + ' in room ' + o.room + ' nickNamed ' + o.nickName,
    )
