...
module.exports.initMod = function(io, gameState, DATA) {
    ...
    setInterval(function() {
        ...
        io.to("cnsnntrm").emit("nonPlayerTalked", {
            id: "",
            labelColor: "#3e7eb0",
            room: "cnsnntrm",
            message: msg,
    ...
    npc.behavior = setTimeout(function ramble() {
        ...
        if (dice < 40) {
            npc.talk(
                global.paranoidTalk[
                    Math.floor(random(0, global.paranoidTalk.length - 1))
                ]
            );
...
module.exports.censorshipRoomTalkFilter = function(player, message) {
    ...
    for (var j = 0; j < words.length; j++) {
        ...
        if (j < words.length - 1) censoredMessage += " ";
    }
...
module.exports.rhymeRoomTalkFilter = function(player, message) {
    //
    if (global.beat == null) global.beat = 1;

    var arr = message.split(" ");
    var lastWord = "";

    if (arr.length > 0) lastWord = arr[arr.length - 1].toLowerCase();
    ...
    if (global.lastRhymes.length == 0) first = true;
    else {
    ...
    if (exists && !used) {
        ...
        if (first || rhymes) {
            ...
            global.rhymeTime = setTimeout(function() {
                ...
                //change record
                global.beat++;
                if (global.beat > 3) global.beat = 1;
                ...
                for (var nn in global.rhymingPlayers) {
                    if (
                        global.rhymingPlayers[nn] >= 4 &&
                        global.rhymingPlayers[nn] > highscore
                    ) {
                        winner = nn;
                ...
                global.rhymingPlayers = {};
                if (winner != "") {
                    io.to("rhymeRoom").emit(
                        "godMessage",
                        winner + " wins!" + highscore + " words uttered"
                    );
                }
            }, 8000);
...
module.exports.familyRoomJoin = function(playerObject, roomId) {
    ...
    if (!foundRole) console.log(playerObject.id + " becomes a fly");
...
module.exports.VIPRoomJoin = function(playerObject, roomId) {
    ...
    if (global.VIPList.length > 3) {
        ...
        if (io.sockets.sockets[expelled] != null) {
            ...
            io.to(expelled).emit(
                "godMessage",
                "Sorry, we had to expel you to make room for " + playerObject.nickName
            );
        }
    }
};
